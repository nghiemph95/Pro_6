...
async function handler(req, res) {
  ...
  try {
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
      console.error("Email credentials not configured (EMAIL_USER/EMAIL_PASS)");
    ...
    const fileList = files.jdFile
      ? Array.isArray(files.jdFile)
        ? files.jdFile
        : [files.jdFile]
      : [];
    ...
    if (firstFile && (firstFile.filepath || firstFile.path)) {
      ...
      if (
        !ALLOWED_EXT.test(ext) ||
        (!ALLOWED_MIMES.includes(mime) && mime !== "")
      ) {
      ...
      if (f.size > 10 * 1024 * 1024) {
        return res
          .status(400)
          .json({ msg: "File size too large. Maximum size is 10MB." });
      }
    ...
    if (uploadedFile && (uploadedFile.filepath || uploadedFile.path)) {
      ...
      mailOptions.attachments = [
        {
          filename:
            uploadedFile.originalFilename || uploadedFile.name || "attachment",
    ...
    const tmpPath =
      uploadedFile && (uploadedFile.filepath || uploadedFile.path);
...
// Disable body parser để formidable có thể parse multipart
